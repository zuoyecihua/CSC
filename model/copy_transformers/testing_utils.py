...
def tooslow(test_case):
    """
    Decorator marking a test as todo slow.

    Slow tests are skipped while they're in the process of being fixed. No test should stay tagged as "tooslow" as
    these will not be tested by the CI.

    """
    return unittest.skip("test is todo slow")(test_case)
...
class CaptureStd:
    ...
    def __init__(self, out=True, err=True):
        if out:
            ...
            self.out = "error: CaptureStd context is unfinished yet, called todo early"
        else:
            self.out_buf = None
        ...
        if err:
            ...
            self.err = "error: CaptureStd context is unfinished yet, called todo early"
        else:
            self.err_buf = None
